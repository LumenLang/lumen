...
right win: Handles 1000 to 5000 line scripts through the MCP, depending on the model and how the request is phrased
:::
:::row Iteration Loop
...
left warn: Common, mostly from silent coercion
right win: Far less, since most mistakes are caught at compile time
:::
:::row Syntax
left warn: Familiar, can get messy on larger scripts
right win: Familiar feel, structured for larger scripts
:::
:::row Performance
...
right warn: Decisions made up front, easier to debug later
:::
:::row Ecosystem
left win: Large, with many addons
right warn: Much smaller, growing
:::
:::row Stability
left win: Matured over a decade
right warn: Beta, but improving fast
:::
:::row Iteration Speed
left warn: Decent
...
This is why Lumen's AI tooling can produce scripts at scales that aren't realistic on Skript.

> A 2000-line, 9-page shop GUI was generated by Sonnet on the first try in about twelve minutes.
>
> The MCP has been used to ship scripts in the 1000 to 5000 line range. Reliability scales with the model and how clearly the request is phrased.

This isn't magic. The AI can still make mistakes. The difference is the compiler usually catches them and the model fixes them before you see the result, instead of you finding out at runtime.
...
#### When AI migration falls short

Lumen is still young, and the most common reason a migration won't go smoothly is that the equivalent
**pattern doesn't exist yet** in Lumen. The AI can't translate a Skript pattern that has no Lumen counterpart, no
matter how clever it is.

When that happens:

- The AI will tell you which line it can't translate, and what the closest available pattern is.
- You can often work around it with a `java:` block (raw Java escape hatch) or by combining smaller patterns.
- The right long-term fix is to
  [open an issue on the issue tracker](https://github.com/LumenLang/lumen/issues) requesting the missing pattern.
  Lumen is solo-developed, but the pattern set grows quickly and user-requested patterns are prioritized.

Don't expect every Skript script to convert cleanly the first time. Expect most to convert cleanly, some to need
a workaround, and the rare one to wait on a missing pattern.
...
If you're reading the differences and thinking "this looks like a lot more upfront work than Skript", the surface
change is real, but most of it isn't actually harder once you've written a few scripts. The English-like syntax keeps
the day-to-day feel close to Skript. What's genuinely different is that Lumen makes you decide things explicitly that
Skript decides silently for you.

Here's why those decisions are made the way they are.
...
runs. That's dynamic typing, and it's why Skript is forgiving up front but unpredictable in practice. Lumen knows the
type of every value before execution. The day-to-day feel is similar, but once you're building real things, you get
far fewer runtime surprises, and the bugs that do appear are much easier to debug.
...
In practice this isn't extra typing, it's a choice you make on the line itself. When you have a player and the next
pattern wants a string, you decide *which* string. The player's name? Their UUID? Their display name? Their world's
name? Skript picks for you, and sometimes picks wrong. Lumen makes you say it once, on the spot, and the rest of the
script reads exactly the way it runs.
...
Lumen feels different upfront because the things that make Skript forgiving upfront are also what make Skript scripts
break in mysterious ways later. The tradeoff is making a few decisions explicit now, far fewer "why did this just stop
working" moments later, and tooling that actually helps you while you write.
