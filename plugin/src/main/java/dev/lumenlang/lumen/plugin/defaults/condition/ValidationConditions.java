...
import dev.lumenlang.lumen.plugin.util.EnumValidation;
...
public final class ValidationConditions {
    ...
    private void registerMaterial(@NotNull LumenAPI api) {
        api.patterns().condition(b -> b
                ...
                .handler(ctx -> {
                    ctx.codegen().addImport(Material.class.getName());
                    return "(Material.matchMaterial(String.valueOf(" + ctx.java("val")
                            + ").toUpperCase()) != null)";
                }));
    ...
    private void registerEntityType(@NotNull LumenAPI api) {
        api.patterns().condition(b -> b
                ...
                .category(Categories.createOrGet("Validation"))
                .handler(ctx -> {
                    ctx.codegen().addImport(EnumValidation.class.getName());
                    return "EnumValidation.isValidEntityType(String.valueOf(" + ctx.java("val") + "))";
                }));
    ...
    private void registerAttribute(@NotNull LumenAPI api) {
        api.patterns().condition(b -> b
                ...
                .handler(ctx -> {
                    ctx.codegen().addImport(EnumValidation.class.getName());
                    return "EnumValidation.isValidAttribute(String.valueOf(" + ctx.java("val") + "))";
                }));
