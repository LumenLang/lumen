...
/**
 * Registers the {@code ATTRIBUTE} type binding, which resolves user-friendly
 * attribute names (e.g. {@code "max_health"}) into the correct
 * {@code org.bukkit.attribute.Attribute} enum constant for the current server
 * version.
 *
 * <p>
 * The binding delegates all name resolution to {@link AttributeNames}, which
 * handles the 1.20 to 1.21.2+ rename transparently.
 *
 * @see AttributeNames
 * @see AttributePatterns
 */
@Registration(order = -900)
@Description("Registers the ATTRIBUTE type binding for version-aware attribute resolution.")
